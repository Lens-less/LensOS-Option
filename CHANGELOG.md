...
本版本将首次学习、期权决策研究、证据复核与开源验证连接成完整工作流。
研究输出继续失败关闭，执行边界保持不变；具体发布和验收状态见 [v0.5.0 交付记录](docs/product/2026-09-08-open-source-decision-platform.md)。

### 新增

- 独立于研究 API 的三步离线学习导览，包含有限风险结构、教学到期收益交互与真实快照入口。
- 固定包内输入与显式时钟的复现工具，支持完整结果重复计算、保存和比对；保留实际信任阻断。
- `AnalysisInputs` 带类型边界、五组准入条件、惰性 `CompatibilityProjection`，以及构建/输入身份。
- 固定公开 OpenAPI/schema 与发布产物合同验证；Python、Web、扩展、tag 与 wheel 版本一致性检查。
- 统一 `tools/verify.py`，涵盖源码、构建产物、干净 wheel 安装和浏览器流程。
- 中英文完整决策研究指南、阻断与过期恢复说明、贡献验证和安装升级文档。

### 修复与调整

- 策略卡成本从 bool 标记收紧为带模型/配置身份的数字预算，净权利金扣除入场费、滑点与分腿预算；
  损失标为 `PAYOFF_BOUND_PLUS_FROZEN_COST_BUDGET`，交割费绝对上界未验证时卡片最高保持 `WATCH`。
- 预测选腿绑定纳入成本与净权利金，旧校准产物不能直接复用；旧 v0.4.0 策略报告需由升级后的引擎
  重新生成，前端和扩展同步升级。
- 历史路径不再从该路径的未来收益推导状态相似权重；缺少路径开始前证据时显式采用无条件等权。
- 路径不利变动按实际腿方向计算，区分短腿行权价穿越与价格阈值代理；未提供动态 Delta 模型。
  压力场景补齐双向冲击，并声明权重不是校准概率。
- 精确结构回放按逐腿规则计算交割费，说明风险分母不含交割费，保留净 `R < -1` 的真实计算结果。
- 信号/序列 API 校验研究标记、schema 与时间，递归拒绝执行和 sizing 字段，保留合法阻断及排除证据；
  旧手工工件需要由当前 CLI 重生成。
- 统一研究简报、候选、公开版、侧栏的当前时效与历史评估口径，过期证据不再显示当前资格。
- 无效 JSON、结构错误、请求超时和渲染异常提供明确恢复入口；失败后撤下旧结果，重试成功后恢复。
- 区分信号/序列产物缺失、读取错误与样本不足，补齐窄屏导航与键盘交互。
- 文档修正 preflight 未落盘、退出码与门禁混淆、缓存到期、日收盘结算代理等说明。
- 公开开发以独立公开仓库历史为基线，私有 archive 的分支与历史继续隔离。

升级方法与边界见 [v0.5.0 发布说明](docs/releases/v0.5.0.md)。
