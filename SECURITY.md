...
- While this repository is private, the scheduled workflow uploads public-market capture artifacts with 90-day retention as an off-device recovery copy. The step is conditioned on GitHub reporting `repository.private == true`; before visibility changes, historical workflow artifacts/runs must still be inventoried and removed or privately archived. Raw capture is not implicitly a public CC BY release; durable backup still requires the separately owned evidence repository.
...
Those artifacts remain reachable in git history prior to that cleanup. History
rewrite is therefore a mandatory precondition before public release; accepting
continued exposure is not an allowed release path. GitHub read-only pull-request
refs and cached views must also be removed with GitHub Support. If that cannot
be verified, publish the rewritten `main` into a new public repository and keep
this repository permanently private.
